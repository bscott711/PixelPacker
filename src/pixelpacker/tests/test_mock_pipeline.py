...
@pytest.mark.mock
def test_pipeline_handles_output_write_error(
    mocker, tmp_path, synthetic_tiff_factory
):
...
@pytest.mark.mock
def test_pipeline_handles_input_read_error(
    mocker, tmp_path, synthetic_tiff_factory
):
    ...
    mock_extract = mocker.patch("pixelpacker.crop.extract_original_volume") # <<< CHANGED TARGET
    ...
    try:
        assert mock_extract.call_count > 0, "extract_original_volume mock was not called!"
    except AssertionError as e:
        pytest.fail(str(e))

    # Assert the exception message contains info about the error
    assert "Invalid TIFF data" in str(excinfo.value) or "processing error" in str(excinfo.value) or "Failed during task prep" in str(excinfo.value) or "No contrast limits" in str(excinfo.value)
...
def test_pipeline_handles_output_mkdir_error(
    ...
    with caplog.at_level(logging.ERROR):
        result = runner.invoke(
            ...
            [ "--input", str(input_dir), "--output", str(output_dir), "--threads=1", ],
        )
