...
try:
    ...
    logging.warning(
        "scikit-image not found. Tiling will use NumPy loop fallback."
    )
# --- End Added import ---
...
def _save_debug_array_as_image(
    ...
    try:
        ...
        if limits:
            ...
            arr_max = np.max(arr)
            if arr_max > arr_min:
                arr_norm = (arr.astype(np.float32) - arr_min) / (
                    arr_max - arr_min
                )
                arr_8bit = (np.clip(arr_norm, 0.0, 1.0) * 255.0).astype(
                    np.uint8
                )
            else:
                # Flat image
        ...
        log.error(
            f"Failed to save debug array image {filename}: {e}", exc_info=True
        )


# --- Z-Cropping Method 1: Simple Threshold on Max Profile ---
def _find_z_crop_range_threshold(
    volume: np.ndarray, threshold: int
) -> Tuple[int, int]:
    """
    ...
    try:
        ...
        log.debug(
            f"Max Z profile range:"
            f" [{np.min(max_z_profile)}, {np.max(max_z_profile)}]"
        )
        ...
        if z_start > z_end:
            log.error(
                f"Threshold method: Z-crop calc start > end"
                f" ({z_start} > {z_end})."
            )
            return (0, depth - 1)
        ...
        log.error(
            "MemoryError calculating MIPs/profile for threshold crop."
            " Skipping crop."
        )
        return (0, depth - 1)
    except Exception as e:
...
def _find_z_crop_range_slope(
    ...
    try:
        ...
        log.debug(
            f"Applying Gaussian smoothing (sigma={SMOOTHING_SIGMA_Z})"
            " to Z profile..."
        )
        ...
        if not found_start:
            log.warning(
                "Could not reliably determine Z start via slope. Using Z=0."
            )
            z_start_found = 0
        if not found_end:
            log.warning(
                f"Could not reliably determine Z end via slope."
                f" Using Z={depth - 1}."
            )
            z_end_found = depth - 1
...
def find_z_crop_range(
    ...
    if method == "slope":
        ...
        return _find_z_crop_range_slope(
            volume, debug, output_folder, filename_prefix
        )
        ...
        if squeezed_vol.ndim == 3:
            ...
            if squeezed_vol.ndim >= 3:
                ...
                if final_vol.ndim != 3:
                 ...
                 raise ValueError(f"Unsupported shape after squeeze: {squeezed_vol.shape}")

        if final_vol.ndim != 3:
            # This should ideally not be reached if logic above is correct
            raise ValueError(
                f"Volume shape is not 3D after processing:"
                f" {final_vol.shape}"
            )

        log.debug(
            f"Final extracted original volume shape: {final_vol.shape}"
        )
        return final_vol
    except Exception as e:
        log.error(
            f"Error extracting {tif_path.name}: {e}", exc_info=True
        )
        return None
        ...
        if np.can_cast(vol_8bit, np.uint8):
            try:
                ...
                if max_val > 0:
                 ...
                 log.error(f"Failed to auto-convert preview input dtype {vol_8bit.dtype} to uint8.")
                 return
        else:
            log.error(f"Cannot safely cast preview input dtype {vol_8bit.dtype} to uint8.")
            return
...
def save_histogram_debug(
    ...
    try:
        ...
        if nonzero_pixels.size == 0:
            log.warning(
                f"No positive pixel values for hist {out_path.name}. Skipping."
            )
            return
        ...
        if not (np.isfinite(vmin) and np.isfinite(vmax)) or vmax <= vmin:
            log.warning(
                f"Invalid range [{vmin}, {vmax}] for hist {out_path.name}."
                " Skipping."
            )
            return
        ...
        plot_markers = {
            ...
            (
                ...
                if limits.smart_early is not None else None
            ): (limits.smart_early, "purple", "--"),
            (
                f"Smart Late ({limits.smart_late:.1f})"
                if limits.smart_late is not None else None
            ...
            (
                ...
                if limits.actual_max is not None else None
            ): (limits.actual_max, "magenta", ":"),
            (
                f"Actual Min ({limits.actual_min:.1f})"
                if limits.actual_min is not None else None
...
def process_channel(
    ...
    try:
        ...
        if not dry_run:
            ...
            log.debug(
                f"T:{time_id} C:{ch_id} - Converting tiled NumPy array"
                " to PIL Image."
            )
            try:
                ...
                log.error(
                    "Failed to create PIL Image from tiled NumPy array:"
                    f" {img_e}",
                    exc_info=True,
                )
                return None
            ...
            try:
                ...
                log.error(
                    f"Failed save WebP {out_path}: {e}", exc_info=True
                )
                return None
        ...
        log.error(
            f"❌ Unexpected Error T:{time_id} C:{ch_id}: {e}", exc_info=True
        )
        result_data = None
